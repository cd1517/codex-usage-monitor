...
<p align="center">
  <a href="https://github.com/cd1517/codex-usage-monitor/releases/latest"><strong>下载最新版本</strong></a>
  ·
  <a href="#终端安装">终端安装</a>
  ·
  <a href="#本地开发">本地开发</a>
...
Codex Usage Monitor 是一个独立的 macOS 菜单栏应用。它把 Codex 的 **5 小时**与**1 周**剩余用量以横条形式无缝贴在 ChatGPT 标题栏上；悬停横条时，详情窗口从横条下方弹出。无需打开设置，也不会抢走输入焦点。
...
```text
Sources/
├── CodexUsageMonitorCore/       # 用量模型、JSON-RPC、窗口与几何逻辑
├── CodexUsageMonitorApp/        # 菜单栏、浮窗和应用生命周期
└── CodexUsageMonitorCheck/      # 只读联调入口
Tests/
└── CodexUsageMonitorCoreTests/  # 无依赖测试运行器
```
