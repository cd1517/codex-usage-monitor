...
- `Sources/CodexUsageMonitorCore/`：用量模型、JSON-RPC、窗口选择与几何计算，必须可测试且不依赖 UI。
- `Sources/CodexUsageMonitorApp/`：应用生命周期、菜单栏、浮窗与系统事件。
- `Tests/CodexUsageMonitorCoreTests/`：核心行为测试。
...
- 浮窗不得抢走 ChatGPT 输入焦点。
- 浮窗仅在 ChatGPT 位于前台时使用浮动层级；切到其他应用后必须立即降回普通层级并允许其自然遮挡。
