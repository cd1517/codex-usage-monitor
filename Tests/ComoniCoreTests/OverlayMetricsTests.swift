...
func runOverlayMetricsTests() throws {
    ...
    try expect(
        defaultMetrics.expandedSize == CGSize(width: 268, height: 202),
        "18pt should include the fixed strip and four detail rows"
    )
    try expect(
        defaultMetrics.detailPanelSize == CGSize(width: 268, height: 170),
    ...
    try expect(
        expandedFrame == CGRect(x: 812, y: 684, width: 268, height: 202),
